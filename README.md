...
Seventeen first-party applications are wired into `PRODUCT_PACKAGES` as meaningful **`ANDROID_SOURCE`**. They remain below `ANDROID_RUNTIME` until the pinned AOSP product actually builds and they are exercised in Cuttlefish.
...
- **Swir Camera** — first-party Camera2 preview with owner-granted `CAMERA` permission, back/front selection, JPEG photo capture, silent H.264/MP4 video capture into scoped MediaStore collections, and a framework-derived camera capability report. It deliberately does not request microphone, network or broad-storage access; runtime and exact-device camera/HAL behavior remain unverified.
...
All seventeen apps ship English, Polish, Norwegian Bokmål, German, Spanish, French, Portuguese and Arabic resources with RTL-aware application layout. Source validation checks package identity, exact per-app permission allowlists, localization parity, pure-Java policy cores and complete bounded AOSP staging. It scans every production Java file for forbidden process/network/broad-storage primitives; SwirRoot receives additional fail-closed checks. Camera is limited to `CAMERA`, Contacts to `READ_CONTACTS`, Gallery to `READ_MEDIA_IMAGES` + `READ_MEDIA_VIDEO`, Recorder to `RECORD_AUDIO`, and all other current source apps — including Swir Phone and Swir Messages — remain permission-free. Android localization lint additionally verifies exact source-key coverage, formatter/plural contracts and common hardcoded Java UI sinks; this remains source-only evidence, not runtime RTL proof.
...
Host localization is data-driven with system-locale detection and English fallback. The desktop and all seventeen current Android source apps use EN/PL/NB/DE/ES/FR/PT/AR catalogs. The mobile architecture requires the same localization discipline for first setup, launcher, SystemUI, Settings, recovery, updater, SwirRoot and all bundled apps, including RTL, fonts/scripts, plurals, accessibility and locale-specific formatting.
