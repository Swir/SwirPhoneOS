...
The repository contains an x86_64 Cuttlefish product, exact-tag workspace planning, resolved `repo manifest -r` verification, bounded `vendor/swir/` source staging and strict build/runtime evidence tooling. Staging records exact per-file size/SHA-256, verifies copied bytes and re-verifies those exact bytes after compilation. `build-evidence` requires hashed core images and the exact pinned Android release/API, build ID `CP2A.260605.016`, security patch `2026-06-05` and `userdebug` type from the built product. Schema-v3 Cuttlefish evidence requires the exact product/device/manufacturer, Android 17 / API 37, every source-ready package and package-local launcher resolution. `evidence-bundle` is accepted only when runtime fingerprint matches the hashed build output.

The manual-only self-hosted `aosp-build-evidence.yml` workflow now also emits a final `aosp-run-evidence.json`. This binds the exact workflow source commit to builder preflight, exact AOSP plan, resolved manifest, pre/post-build staged-source evidence and build evidence; when runtime is requested, runtime/smoke/bundle evidence must be supplied as one exact group using the same fingerprint and source-ready package set. Cross-run mixing is rejected. The workflow's preflight gate consumes the real `checks[].id` / `checks[].passed` schema and fails before source synchronization on inconsistent or failed mandatory host checks.

When runtime collection is explicitly enabled, the workflow can launch the **exact product it just built** with Cuttlefish, wait for strict verified boot evidence, run emulator-only launch smoke over every source-ready app, bind build/runtime fingerprints, bind the complete run evidence chain, and execute `stop_cvd` in cleanup. This path is implemented but has **not** completed a real AOSP build/boot yet, so weighted platform/runtime gates remain open.
