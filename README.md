...
- **Swir Camera source-stage capture:** exactly `CAMERA` permission; owner-triggered Camera2 live preview and JPEG still capture save through scoped MediaStore, while a direct silent H.264/MP4 path uses `MediaCodec` + `MediaMuxer`, bounded encoder settings and pending MediaStore rows under `Movies/SwirPhoneOS`; unsupported direct encoder surfaces keep a visible Android video-capture hand-off. Runtime/photo/video-quality and exact-device camera claims remain blocked until real validation, and `camera:video_capture` is not capability-credited yet.
...
| Camera | Exactly `CAMERA`; first-party Camera2 live preview, front/back selection and owner-triggered direct JPEG still capture with scoped pending-row MediaStore save/cleanup under `Pictures/SwirPhoneOS`; direct source-stage H.264/MP4 video without audio uses `MediaCodec` + `MediaMuxer`, bounded even dimensions/bitrate, five-second finalization timeout and pending MediaStore save/cleanup under `Movies/SwirPhoneOS`, with a visible Android capture hand-off when no direct encoder surface is available; runtime, photo/video quality and exact-device behavior remain unverified and `video_capture` remains open |
