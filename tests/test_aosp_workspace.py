...
class AospWorkspaceTests(unittest.TestCase):
    ...
    def test_plan_is_pinned_and_never_claims_device_write(self) -> None:
    ...
    def test_plan_rejects_unpinned_baseline(self) -> None:
    ...
    def test_plan_rejects_root_workspace_and_bad_jobs(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
    ...
    def test_manifest_requires_every_project_to_be_full_sha(self) -> None:
        xml = """<manifest>
<project name="platform/build" path="build" revision="1111111111111111111111111111111111111111" />
<project name="platform/frameworks/base" path="frameworks/base" revision="2222222222222222222222222222222222222222" />
</manifest>"""
        with tempfile.TemporaryDirectory() as temp:
    ...
    def test_manifest_rejects_floating_revision_and_duplicates(self) -> None:
        floating = "<manifest><project name='x' path='x' revision='main'/></manifest>"
        duplicate = """<manifest>
<project name="x" path="x" revision="1111111111111111111111111111111111111111" />
<project name="y" path="x" revision="2222222222222222222222222222222222222222" />
</manifest>"""
        with tempfile.TemporaryDirectory() as temp:
            path = Path(temp) / "resolved.xml"
    ...
    def test_staging_is_dry_run_by_default_and_requires_aosp_checkout_to_execute(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            product.mkdir()
            for name in ("AndroidProducts.mk", "swirphoneos_cf_x86_64.mk"):
                (product / name).write_text("test\n", encoding="utf-8")
            workspace = root / "aosp"
