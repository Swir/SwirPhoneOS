...
class AndroidAppSourceTests(unittest.TestCase):
    def test_repository_source_apps_are_complete_but_not_runtime_claimed(self):
        ...
        self.assertEqual(set(summary["source_ready_apps"]),{"calculator","settings","files","device_care","update","privacy","clock","notes","calendar"})
        self.assertEqual(summary["source_ready_count"],9); self.assertEqual(summary["localized_catalogs"],72)
        for capability in ("basic_math","system_settings","search","device_status","browse","copy_move_rename","share","safe_delete","storage_status","battery_status","thermal_status","hardware_diagnostics","channel_status","signed_metadata","permission_review","alarms","timers","stopwatch","world_clock","offline_notes","export","local_calendar"):
            self.assertIn(capability,summary["implemented_capabilities"])
        self.assertEqual(summary["remaining_target_capabilities"],["access_history","privacy_indicators","provider_bridge","recovery_handoff","scientific_math","staged_update_state"])
    ...
    def _copy_fixture(self):
        temp=tempfile.TemporaryDirectory(); root=Path(temp.name); product=root/"product"; shutil.copytree(Path("platform/aosp_product"),product); registry=root/"manifest.json"; shutil.copy2(Path("system_apps/manifest.json"),registry); return temp,product,registry
