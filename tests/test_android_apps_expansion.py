...
class AndroidAppExpansionTests(unittest.TestCase):
    ...
    def test_all_essential_apps_are_reviewed_source_not_runtime(self):
        ...
        for capability in ("dialer", "in_call", "recent_calls", "sms", "camera_capability_report", "web_browsing", "downloads", "privacy_controls", "forecast", "provider_attribution", "unit_preferences", "supported_data_backup", "recovery_metadata", "offline_notes", "local_calendar", "provider_bridge", "local_media", "audio_recording", "microphone_state", "file_export", "local_contacts", "import_export", "package_catalog", "signature_provenance", "root_state", "authorization_audit"):
            self.assertIn(capability, summary["implemented_capabilities"])
        for capability in ("mms", "conversation_history", "photo_capture", "video_capture", "restore_orchestration", "albums", "update_status", "guided_enable", "guided_unroot"):
            self.assertIn(capability, summary["remaining_target_capabilities"])
        self.assertNotIn("in_call", summary["remaining_target_capabilities"])
        self.assertNotIn("recent_calls", summary["remaining_target_capabilities"])
