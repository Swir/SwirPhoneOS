...
class AndroidAppExpansionTests(unittest.TestCase):
    ...
    def test_notes_and_calendar_are_reviewed_source_not_runtime(self):
        summary = public_android_app_source_summary(validate_android_app_sources())
        self.assertEqual(summary["source_ready_count"], 9)
        self.assertEqual(summary["localized_catalogs"], 72)
        self.assertIn("notes", summary["source_ready_apps"])
        self.assertIn("calendar", summary["source_ready_apps"])
        self.assertIn("offline_notes", summary["implemented_capabilities"])
        self.assertIn("local_calendar", summary["implemented_capabilities"])
        self.assertIn("provider_bridge", summary["remaining_target_capabilities"])
    ...
    def test_stage_fragment_duplicate_is_rejected_across_manifests(self):
        temp, product, registry = self._copy_fixture()
        with temp:
            fragment = product / "stage_manifest.d/notes-calendar.json"
            data = json.loads(fragment.read_text(encoding="utf-8"))
            data["files"][0]["destination"] = "vendor/swir/apps/SwirCalculator/Android.bp"
