...
A separate manual `AOSP builder admission` workflow now performs a read-only host admission on the dedicated runner before expensive sync/build work. It uses the same `swir-aosp-builder` concurrency group, validates the configured absolute workspace through `build-preflight`, enforces the requested job budget and optional KVM requirement, uploads a bounded report, and contains no repo sync, ADB, Fastboot or device-write path. No real admission run has been captured yet.
...
The validator uses exact least-privilege permission allowlists. Swir Phone is limited to `READ_CALL_LOG`, Swir Camera to `CAMERA`, Contacts to `READ_CONTACTS`, Gallery to `READ_MEDIA_IMAGES` + `READ_MEDIA_VIDEO`, Recorder to `RECORD_AUDIO`, and Browser/Weather to `INTERNET`; the remaining source apps are permission-free. Swir Phone does not request `CALL_PHONE` or `WRITE_CALL_LOG`, and its recent-call path has no provider insert/update/delete primitive. Swir Camera does not request storage, network or audio permissions; direct still capture is source-accounted only behind Camera2/MediaStore safety contracts, while direct video stays intentionally silent and focused-test-guarded without widening to `RECORD_AUDIO`. Network primitives are rejected outside the reviewed Browser/Weather paths; broad storage and process execution remain rejected across production Java source. Every app has EN/PL/NB/DE/ES/FR/PT/AR resources with exact key/formatter/plural checks and RTL-aware application configuration.

Camera now has source-implemented owner-triggered Camera2 live preview and direct JPEG still capture with front/back selection and scoped pending-row MediaStore persistence under `Pictures/SwirPhoneOS`; failed save paths delete the pending row. It also has a direct source-stage H.264/MP4 path using `MediaCodec` + `MediaMuxer`, bounded even dimensions/bitrate, orientation binding, serialized setup/record/finalize transitions, a five-second encoder-finalization timeout and pending-row MediaStore persistence under `Movies/SwirPhoneOS`. Unsupported direct encoder surfaces retain an owner-visible Android capture hand-off. `photo_capture` is source-accounted, but Android runtime behavior, photo/video quality and exact-device camera behavior remain unverified; `video_capture` deliberately stays open until the direct path is built and reviewed in Android runtime and later on exact supported hardware. Browser provides HTTPS-first browsing with conservative defaults plus app-scoped owner-visible `DownloadManager` downloads; real redirect/provider behavior remains runtime-unverified. Weather provides bounded HTTPS Open-Meteo forecast retrieval for owner-entered coordinates without location permission. Swir Backup now creates bounded owner-selected schema-v2 document archives with per-file SHA-256, strict inspection and SAF restore into an explicitly chosen folder. Legacy schema-v1 backup archives remain inspect-only. This document restore path does not read private app data or partitions and does not satisfy device install/recovery rollback orchestration.

Phone now has source-implemented owner-controlled default-dialer role request, in-call answer/reject/end controls and bounded read-only recent-call history. Recent calls require the default-dialer role plus an explicit runtime `READ_CALL_LOG` grant, cap output at 20 entries, hide restricted/private caller presentation and never mutate the call-log provider. Real Telecom/modem/IMS behavior remains runtime- and hardware-unverified. Messages still has `mms` and `conversation_history` open; Camera keeps `video_capture` open despite the guarded direct-video source; Calendar's owner-visible `provider_bridge` is source-implemented but remains runtime/provider-app unverified; Backup keeps the broader `restore_orchestration` target open beyond its current owner-selected document restore. Apps now source-implements local installed `update_status` classification through permission-free PackageManager metadata, live resume refresh and bounded sanitized display metadata; remote catalog checks, downloads, package installation and Android runtime behavior remain open. SwirRoot keeps guided enable/unroot open. Source-summary schema v3 tracks these gaps per app.
...
- [ ] Swir Camera direct still capture and direct H.264/MP4 video path validated against the exact reference camera/media stack before runtime/hardware capabilities are promoted.
...
4. Perform focused interactive checks automated launch/locale smoke cannot prove: core Settings/Files/Update/Privacy/DeviceCare flows; Phone default-role request, `READ_CALL_LOG` permission denial/grant, restricted-number privacy, recent-call list and in-call controls; Messages hand-off; Camera runtime `CAMERA` permission denial/grant, live preview, front/back switch, direct JPEG capture, photo MediaStore save/failure cleanup, direct silent H.264/MP4 start/stop/repeated-tap/finalization, video pending-row cleanup, orientation and unsupported-direct-surface fallback; Browser HTTPS/privacy/download behavior; Weather network/error/unit behavior; Backup schema-v2 create/inspect/restore plus failure cleanup; Notes/Calendar persistence/export; Clock timer/alarm; Gallery; Recorder; Contacts; Swir Apps; SwirRoot; accessibility, text expansion, locale-specific formatting/input/fonts and Arabic RTL visual mirroring. Only then review any `ANDROID_RUNTIME` promotion.
