...
_SPECS = {
    ...
    "gallery": _AppSpec("gallery", "SwirGallery", "SwirGallery", "org.swir.phoneos.gallery", "src/org/swir/phoneos/gallery/MediaPolicy.java", "src/org/swir/phoneos/gallery/MainActivity.java", "hosttest/MediaPolicyHostTest.java", "res/drawable/ic_gallery.xml", ("local_media", "albums", "share", "safe_delete"), ("android.permission.READ_MEDIA_IMAGES", "android.permission.READ_MEDIA_VIDEO")),
...
def _load_stage_sources(product_root: Path) -> set[str]:
    try:
        files = load_stage_files(product_root)
    except StageManifestError as exc:
        raise AndroidAppSourceError(str(exc)) from exc
    if not files:
...
def _validate_common(product_root: Path, product_mk: str, app, spec: _AppSpec, staged: set[str]) -> tuple[int, int, str, str, str]:
    ...
    if not java_files:
        raise AndroidAppSourceError(f"{spec.module} has no Android Java source.")
    ...
    if any(token in java_bundle or token in manifest_text for token in forbidden):
        raise AndroidAppSourceError(f"{spec.module} source contains a forbidden execution/storage primitive.")
    network_tokens = ("java.net.", "javax.net.", "android.webkit.", "loadUrl(")
    if spec.app_id not in _NETWORK_APPS and any(token in java_bundle for token in network_tokens):
        raise AndroidAppSourceError(f"{spec.module} source contains an unreviewed network primitive.")
    if spec.app_id not in _NETWORK_APPS and "android.permission.INTERNET" in manifest_text:
        raise AndroidAppSourceError(f"{spec.module} may not request network access.")
    english = _string_keys(app_root / "res/values/strings.xml")
    localized = 0
    for locale in _LOCALES:
        if _string_keys(app_root / "res" / _RESOURCE_DIR[locale] / "strings.xml") != english:
...
def _validate_phone(logic, activity):
    if any(x not in logic for x in ("MAX_DIAL_LENGTH", "normalize", "isDialable", "appendKey", "eraseLast", "Character.isDigit")):
        raise AndroidAppSourceError("SwirPhone host-tested dial policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirPhone must retain explicit owner-visible dialing plus default-role-gated read-only recent-call history.")
    ...
    if any(x in activity for x in forbidden):
        raise AndroidAppSourceError("SwirPhone must not directly place calls or mutate call history.")


def _validate_messages(logic, activity):
    if any(x not in logic for x in ("MAX_RECIPIENTS", "MAX_BODY_LENGTH", "normalizeRecipients", "normalizeRecipient", "normalizeBody", "canHandoff", "remainingCharacters", "Character.isDigit")):
        raise AndroidAppSourceError("SwirMessages host-tested compose policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirMessages must retain local draft persistence and explicit user-visible system messaging handoff.")
    ...
    if any(x in activity or x in logic for x in forbidden):
        raise AndroidAppSourceError("SwirMessages source stage must not silently send/read SMS or claim conversation history.")
...
def _validate_calculator(logic, activity):
    logic_required = ("java.math.BigDecimal", "equalsResult", "ScientificOperation", "AngleMode", "scientific(", "inputPi", "inputE", "Math.sin", "Math.cos", "Math.tan", "Math.log", "Math.log10")
    if any(x not in logic for x in logic_required):
        raise AndroidAppSourceError("SwirCalculator must retain its host-tested basic and scientific engine.")
    activity_required = ("DecimalFormatSymbols", "scientificButton", "CalculatorEngine.ScientificOperation.SIN", "CalculatorEngine.ScientificOperation.COS", "CalculatorEngine.ScientificOperation.TAN", "CalculatorEngine.ScientificOperation.SQRT", "CalculatorEngine.ScientificOperation.LN", "CalculatorEngine.ScientificOperation.LOG10", "CalculatorEngine.ScientificOperation.RECIPROCAL", "constantButton", "engine.toggleAngleMode()")
    if any(x not in activity for x in activity_required):
        raise AndroidAppSourceError("SwirCalculator must preserve locale-aware scientific controls and angle-mode UI.")


def _validate_settings(logic, activity):
    if frozenset(re.findall(r'"(android\.settings\.[A-Z_]+)"', logic)) != _SETTINGS_ACTIONS:
        raise AndroidAppSourceError("SwirSettings routes must match the reviewed public-settings allowlist exactly.")
    if any(x not in activity for x in ("Build.MODEL", "Build.VERSION.RELEASE", "Build.VERSION.SDK_INT", "startActivity(new Intent(entry.action()))")):
        raise AndroidAppSourceError("SwirSettings must expose real device state and reviewed routes.")
...
def _validate_files(logic, activity):
    ...
    if any(x not in activity for x in required) or "validName" not in logic or "Character.isISOControl" not in logic:
        raise AndroidAppSourceError("SwirFiles must retain its reviewed user-granted SAF workflow and file policy.")
...
def _validate_browser(logic, activity):
    ...
    if any(x not in logic for x in policy_required):
        raise AndroidAppSourceError("SwirBrowser host-tested URL/download policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirBrowser must retain HTTPS-first browsing, conservative privacy defaults and app-scoped owner-visible downloads.")
    forbidden = ("getCookie(", "setDestinationInExternalPublicDir", "setDestinationUri(")
    if any(x in activity for x in forbidden):
        raise AndroidAppSourceError("SwirBrowser downloads must not copy WebView cookies or escape the reviewed app-scoped destination.")
...
def _validate_device_care(logic, activity):
    ...
    if any(x not in activity for x in required) or any(x not in logic for x in ("percentUsed", "thermalBand", "batteryLevelValid")):
        raise AndroidAppSourceError("SwirDeviceCare must retain real framework health state and calculations.")


def _validate_update(logic, activity):
    if any(x not in logic for x in ("SHA256withRSA", 'MessageDigest.getInstance("SHA-256")', "channelForBuild", "fingerprintMatches")) or any(x not in activity for x in ("Build.FINGERPRINT", "Build.TYPE", "Build.TAGS", "Settings.ACTION_SYSTEM_UPDATE_SETTINGS")):
        raise AndroidAppSourceError("SwirUpdate verification/build-state contract drifted.")
...
def _validate_privacy(logic, activity):
    if frozenset(re.findall(r'"(android\.settings\.[A-Z_]+)"', logic)) != _PRIVACY_ACTIONS:
        raise AndroidAppSourceError("SwirPrivacy routes must match the reviewed privacy-settings allowlist exactly.")
    if any(x not in activity for x in ("PrivacyCatalog.search", "resolveActivity(getPackageManager())", "startActivity(intent)")):
        raise AndroidAppSourceError("SwirPrivacy must use reviewed Android settings surfaces.")


def _validate_clock(logic, activity):
    if any(x not in logic for x in ("timerMillis", "validAlarmTime", "stopwatchElapsed", "timerRemaining", "worldTime")) or any(x not in activity for x in ("SystemClock.elapsedRealtime", "AlarmClock.ACTION_SET_ALARM", "AlarmClock.EXTRA_HOUR", "AlarmClock.EXTRA_MINUTES", "AlarmClock.EXTRA_SKIP_UI, false", "ClockCore.timerRemaining", "ClockCore.stopwatchElapsed", "ClockCore.worldTime")):
        raise AndroidAppSourceError("SwirClock daily clock contract drifted.")


def _validate_notes(logic, activity):
    if any(x not in logic for x in ("validNote", "matches", "safeExportBase", "exportMarkdown", "MAX_BODY")):
        raise AndroidAppSourceError("SwirNotes host-tested note policy drifted.")
    if any(x not in activity for x in ("SQLiteOpenHelper", "Intent.ACTION_CREATE_DOCUMENT", "Intent.ACTION_SEND", "openOutputStream", "NotePolicy.exportMarkdown")):
        raise AndroidAppSourceError("SwirNotes must retain local CRUD plus explicit share/export workflows.")
...
def _validate_calendar(logic, activity):
    ...
    if any(x not in logic for x in policy_required):
        raise AndroidAppSourceError("SwirCalendar event/ICS policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirCalendar must retain local-first import/export plus owner-visible device-calendar hand-off.")
    ...
    if any(x in activity for x in forbidden):
        raise AndroidAppSourceError("SwirCalendar must not silently read or mutate the device calendar provider.")


def _validate_weather(logic, activity):
    if any(x not in logic for x in ("validLatitude", "validLongitude", "forecastUrl", "providerUrl", "temperatureUnit", "windUnit")):
        raise AndroidAppSourceError("SwirWeather host-tested forecast policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirWeather must retain bounded HTTPS forecast retrieval, owner-selected coordinates and provider attribution.")
    if any(x in activity for x in ("LocationManager", "FusedLocationProviderClient", "ACCESS_FINE_LOCATION", "ACCESS_COARSE_LOCATION")):
        raise AndroidAppSourceError("SwirWeather source stage must not collect device location without a reviewed permission flow.")


def _validate_gallery(logic, activity):
    logic_required = ("supportedMime", "matches", "isVideo", "safeEpochSeconds", "safeAlbumName", "MAX_ALBUM_NAME_LENGTH", "Character.isISOControl", "album.contains(needle)")
    if any(x not in logic for x in logic_required):
        raise AndroidAppSourceError("SwirGallery host-tested media and album policy drifted.")
    activity_required = ("Manifest.permission.READ_MEDIA_IMAGES", "Manifest.permission.READ_MEDIA_VIDEO", "MediaStore.Files.getContentUri", "MediaStore.Images.ImageColumns.BUCKET_ID", "MediaStore.Images.ImageColumns.BUCKET_DISPLAY_NAME", "MediaPolicy.safeAlbumName", "selectedAlbumId", "renderAlbums", "MediaStore.createDeleteRequest", "startIntentSenderForResult", "Intent.ACTION_VIEW", "Intent.ACTION_SEND", "MediaPolicy.matches")
    if any(x not in activity for x in activity_required):
        raise AndroidAppSourceError("SwirGallery must retain scoped MediaStore album browsing/share and owner-confirmed delete flows.")


def _validate_recorder(logic, activity):
    if any(x not in logic for x in ("recordingFileName", "isRecordingFile", "canExport", "canRecord", "MAX_EXPORT_BYTES")):
        raise AndroidAppSourceError("SwirRecorder host-tested recording policy drifted.")
    if any(x not in activity for x in ("Manifest.permission.RECORD_AUDIO", "requestPermissions", "MediaRecorder.AudioSource.MIC", "MediaRecorder.OutputFormat.MPEG_4", "MediaRecorder.AudioEncoder.AAC", "recorder.pause()", "recorder.resume()", "getFilesDir()", "AudioManager", "isMicrophoneMute", "Intent.ACTION_CREATE_DOCUMENT", "openOutputStream", "AlertDialog.Builder", "stopRecording(false)")):
        raise AndroidAppSourceError("SwirRecorder must retain foreground-only recording, microphone state, private storage and explicit export/delete flows.")


def _validate_contacts(logic, activity):
    if any(x not in logic for x in ("validLookupKey", "matches", "safeExportBase", "vcardFileName")):
        raise AndroidAppSourceError("SwirContacts host-tested contact policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirContacts must retain scoped provider browse/search plus explicit import/export hand-offs.")


def _validate_backup(logic, activity):
    if any(x not in logic for x in ("MAX_FILES", "MAX_ENTRY_BYTES", "MAX_TOTAL_BYTES", "validEntryName", "safeEntryName", "backupFileName", "sizeAllowed")):
        raise AndroidAppSourceError("SwirBackup host-tested archive policy drifted.")
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirBackup must retain owner-selected bounded archive creation and safe structural inspection.")
    if any(x in activity for x in ("ZipFile.extract", "FileOutputStream", "getDataDirectory()", "/data/")):
        raise AndroidAppSourceError("SwirBackup source stage must not restore arbitrary archive entries or read app-private data.")
...
def _validate_swirroot(logic: str, activity: str, java_bundle: str) -> None:
    if any(token not in logic for token in ("State.UNAVAILABLE", "exactBuildMatch", "verifiedDeviceProfile", "ownerConfirmed", "rollbackMaterialVerified", "journalAvailable", "updateStateSafe", "expectedNonRootStateKnown", "writeBackendEnabled", "supportedBuild", "evaluateEnable", "evaluateUnroot")):
        raise AndroidAppSourceError("SwirRoot must retain the complete fail-closed enable/unroot gate model.")
    if any(token not in activity for token in ("AlertDialog.Builder", "bindService", "SwirRootService.class", "service.reviewEnable(true)", "service.reviewUnroot(true)", "service.currentBuildFingerprint()", "R.string")):
        raise AndroidAppSourceError("SwirRoot must retain explicit owner review and localized status UI.")
    if any(token not in java_bundle for token in ("class SwirRootService extends Service", "WRITE_BACKEND_ENABLED = false", "SUPPORTED_BUILD = false", "RootPolicy.State.UNAVAILABLE", "auditSnapshot()", "reviewEnable", "reviewUnroot")):
        raise AndroidAppSourceError("SwirRoot source stage must retain its non-exported fail-closed status/audit service.")
    if any(token in java_bundle for token in ("RecoverySystem.installPackage", "/dev/block/", "android.os.SystemProperties", "bootctl", "setprop", "flash ", "erase ")):
        raise AndroidAppSourceError("SwirRoot source stage must not contain a root/device mutation primitive.")


_VALIDATORS = {
    "phone": _validate_phone, "messages": _validate_messages, "camera": _validate_camera,
    "calculator": _validate_calculator, "settings": _validate_settings, "files": _validate_files,
    "browser": _validate_browser, "device_care": _validate_device_care, "update": _validate_update,
    "privacy": _validate_privacy, "clock": _validate_clock, "notes": _validate_notes,
    "calendar": _validate_calendar, "weather": _validate_weather, "gallery": _validate_gallery,
    "recorder": _validate_recorder, "contacts": _validate_contacts, "backup": _validate_backup,
    "apps": _validate_apps,
}
...
def validate_android_app_sources(product_root: Path = Path("platform/aosp_product"), registry_path: Path = Path("system_apps/manifest.json")) -> AndroidAppSourceSummary:
    ...
    for app in source_apps:
        ...
        count, staged_count, logic, activity, java_bundle = _validate_common(product_root, product_mk, app, spec, staged)
        if app.app_id == "swirroot":
            _validate_swirroot(logic, activity, java_bundle)
        else:
...
def public_android_app_source_summary(summary: AndroidAppSourceSummary) -> dict[str, object]:
    ...
    return {
        ...
        "remaining_app_capabilities": sorted(summary.missing_app_capabilities),
