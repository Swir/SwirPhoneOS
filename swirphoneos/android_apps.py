...
def _validate_calculator(logic,activity):
    ...
    if any(x not in logic for x in logic_required): raise AndroidAppSourceError("SwirCalculator must retain its host-tested basic and scientific engine.")
    ...
    if any(x not in activity for x in activity_required): raise AndroidAppSourceError("SwirCalculator must preserve locale-aware scientific controls and angle-mode UI.")
...
def _validate_gallery(logic,activity):
    ...
    if any(x not in logic for x in logic_required): raise AndroidAppSourceError("SwirGallery host-tested media and album policy drifted.")
    ...
    if any(x not in activity for x in activity_required): raise AndroidAppSourceError("SwirGallery must retain scoped MediaStore album browsing/share and owner-confirmed delete flows.")
