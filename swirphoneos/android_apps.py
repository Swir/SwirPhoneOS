...
_SPECS = {
    ...
    "camera": _AppSpec("camera", "SwirCamera", "SwirCamera", "org.swir.phoneos.camera", "src/org/swir/phoneos/camera/CameraPolicy.java", "src/org/swir/phoneos/camera/MainActivity.java", "hosttest/CameraPolicyHostTest.java", "res/drawable/ic_camera.xml", ("photo_capture", "video_capture", "camera_capability_report"), ("android.permission.CAMERA",)),
...
def _validate_phone(logic: str, activity: str) -> None:
    ...
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirPhone must retain explicit user-visible system dialer handoff and the reviewed keypad policy.")
    if any(x in activity for x in ("Intent.ACTION_CALL", "TelecomManager.placeCall", "CallLog.Calls", "Manifest.permission.CALL_PHONE")):
        raise AndroidAppSourceError("SwirPhone source stage must not place calls directly or claim call-log/in-call support.")


def _validate_messages(logic: str, activity: str) -> None:
...
def _validate_camera(logic: str, activity: str) -> None:
    policy = ("MAX_MEDIA_NAME_LENGTH", "MIN_VIDEO_DURATION_MS", "normalizeRotation", "pickBestSizeIndex", "safeMediaName", "canCapture", "canStopVideo")
    if any(x not in logic for x in policy):
        raise AndroidAppSourceError("SwirCamera host-tested capture policy drifted.")
    required = ("Manifest.permission.CAMERA", "requestPermissions", "CameraManager", "CameraDevice", "CameraCaptureSession", "ImageReader", "CameraDevice.TEMPLATE_STILL_CAPTURE", "MediaStore.Images.Media.getContentUri", "MediaRecorder", "CameraDevice.TEMPLATE_RECORD", "MediaStore.Video.Media.getContentUri", "MediaStore.MediaColumns.RELATIVE_PATH" if False else "MediaStore.Video.Media.RELATIVE_PATH", "CameraPolicy.safeMediaName", "CameraPolicy.canCapture", "CameraPolicy.canStopVideo", "CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL")
    if any(x not in activity for x in required):
        raise AndroidAppSourceError("SwirCamera must retain its owner-granted Camera2 preview/photo/silent-video and capability-report flow.")
    forbidden = ("Manifest.permission.RECORD_AUDIO", "MediaRecorder.AudioSource", "MediaStore.ACTION_IMAGE_CAPTURE", "MediaStore.ACTION_VIDEO_CAPTURE", "Intent.ACTION_CAMERA_BUTTON")
    if any(x in activity or x in logic for x in forbidden):
        raise AndroidAppSourceError("SwirCamera source stage must not record audio or delegate capture through an unreviewed path.")
...
def _validate_notes(logic: str, activity: str) -> None:
...
def _validate_calendar(logic: str, activity: str) -> None:
    if any(x not in logic for x in ("normalizeEnd", "matches", "escapeIcs", "toIcs", "BEGIN:VCALENDAR")):
        raise AndroidAppSourceError("SwirCalendar event/ICS policy drifted.")
    if any(x not in activity for x in ("SQLiteOpenHelper", "DatePickerDialog", "TimePickerDialog", "Intent.ACTION_CREATE_DOCUMENT", "Intent.ACTION_SEND", "EventPolicy.toIcs")) or "CalendarContract" in activity:
        raise AndroidAppSourceError("SwirCalendar must remain local-first until provider integration is reviewed.")


def _validate_gallery(logic: str, activity: str) -> None:
    if any(x not in logic for x in ("supportedMime", "matches", "isVideo", "safeEpochSeconds")):
        raise AndroidAppSourceError("SwirGallery host-tested media policy drifted.")
    if any(x not in activity for x in ("Manifest.permission.READ_MEDIA_IMAGES", "Manifest.permission.READ_MEDIA_VIDEO", "MediaStore.Files.getContentUri", "MediaStore.createDeleteRequest", "startIntentSenderForResult", "Intent.ACTION_VIEW", "Intent.ACTION_SEND", "MediaPolicy.matches")):
        raise AndroidAppSourceError("SwirGallery must retain scoped MediaStore browse/share and owner-confirmed delete flows.")


def _validate_recorder(logic: str, activity: str) -> None:
...
def _validate_contacts(logic: str, activity: str) -> None:
...
def _validate_apps(logic: str, activity: str) -> None:
...
_VALIDATORS = {
    ...
    "calculator": _validate_calculator,
    "settings": _validate_settings,
    "files": _validate_files,
    "device_care": _validate_device_care,
