...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Bind one support-readiness report to exact local physical validation evidence. "
            "The result is review-only and never authorizes support, installation, root or device writes."
        )
    )
    ...
    parser.add_argument("--evidence-root", type=Path, required=True, help="Absolute non-symlink directory containing referenced evidence files")
    ...
    try:
        ...
        print(
            "Physical validation failed: readiness, exact identity/build binding, record schema or referenced evidence is invalid.",
            file=sys.stderr,
        )
        return 1
