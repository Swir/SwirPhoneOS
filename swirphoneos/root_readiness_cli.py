...
def main(argv: list[str] | None = None) -> int:
    ...
    except (SwirRootReadinessError, SwirRootPolicyError, TransactionEvidenceError, HardwareEvidenceError, OSError, ValueError):
        print(
            "SwirRoot readiness failed: evidence is missing, contradictory, malformed, or not bound to the requested exact build.",
            file=sys.stderr,
        )
        return 1
