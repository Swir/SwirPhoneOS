...
### Swir Camera
Requests exactly `CAMERA` and owns first-party Camera2 live preview plus owner-triggered direct JPEG still capture with front/back selection. Photos are written through a scoped pending MediaStore row under `Pictures/SwirPhoneOS` and failed writes delete the pending row. The source also contains an owner-started direct H.264/MP4 video path using `MediaCodec` + `MediaMuxer`, bounded even dimensions/bitrate, a five-second encoder-finalization timeout and a scoped pending MediaStore row under `Movies/SwirPhoneOS`; it intentionally records without audio and therefore does not request `RECORD_AUDIO`. When the selected camera does not expose a usable encoder surface, the app retains a visible `ACTION_VIDEO_CAPTURE` hand-off instead of inventing direct support. `photo_capture` is source-accounted. `video_capture` deliberately remains open until this media path is built and reviewed in the pinned Android runtime and later validated on exact supported hardware.
...
Source validation requires package identity, exact least-privilege permission allowlists, product inclusion and complete bounded AOSP staging. Swir Phone is allowed only `READ_CALL_LOG`; Contacts only `READ_CONTACTS`; Gallery only `READ_MEDIA_IMAGES` + `READ_MEDIA_VIDEO`; Recorder only `RECORD_AUDIO`; Browser and Weather only `INTERNET`; Swir Camera only `CAMERA`. The remaining source apps are permission-free. Network primitives are reviewed only for Swir Browser and Swir Weather; other apps fail closed on unreviewed network code. Broad-storage and process-execution primitives remain rejected. Swir Phone additionally rejects direct-call and call-log-write primitives, Swir Camera rejects microphone/audio-permission expansion in its direct-video source stage, and SwirRoot receives additional hard-disabled mutation checks.

Source-summary schema v3 records missing capabilities by **app + capability**. Swir Phone `dialer`, `in_call` and bounded read-only `recent_calls`, Swir Camera `photo_capture` and `camera_capability_report`, Swir Gallery `albums`, and Swir Calculator `scientific_math` are source-implemented. The direct Swir Camera H.264/MP4 implementation is present and focused-test-guarded, but `camera:video_capture` remains intentionally open until the exact built Android runtime is reviewed. Other important open items include `messages:mms`, `messages:conversation_history`, `backup:restore_orchestration`, `privacy:privacy_indicators`, `privacy:access_history`, `update:staged_update_state`, `update:recovery_handoff`, `apps:update_status`, `swirroot:guided_enable` and `swirroot:guided_unroot`.
