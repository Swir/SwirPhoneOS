...
A green admission is **not build evidence** and is **not boot evidence**. It only removes uncertainty about whether the dedicated builder satisfies the checked-in host/workspace prerequisites and preserves the exact host/toolchain identity snapshot for later continuity verification. The canonical build, boot and runtime evidence must still come from the separate **AOSP build evidence** workflow and its bound artifacts.
