...
`ClockCoreHostTest` exercises the exact reviewed zone inventory, wraparound behavior, invalid-index rejection, invalid-IANA-zone rejection, UTC/Tokyo time conversion and localized zone labels. `tests/test_clock_world_clock.py` additionally checks the Android source hand-off, permission boundary and all eight catalogs.
